...
public class LookBridge {
	...
	public static void main(String[] args) throws Exception {
		//String file = args[0];
		String file = "LookBridge.csv";
		LookBridge bridge = new LookBridge();
		Map<String, Object> result = bridge.readDaily(file);
		
		System.out.println("Last Date = "+ lastDate);
		Map<String, String> lastData = (Map<String, String>) result.get(lastDate);
		
		//look bridge
		Map<String, Object> ngay_bridges = bridge.lookBridge(result);
		...
		//print out
		for(Entry<String, Object> _bridge : ngay_bridges.entrySet()) {
...
//		}
		...
		for(Entry<String, String> entry : giai_of_ngay_bigger_than_3.entrySet()){
			...
			String realKey = key.substring(0, key.indexOf("_index"));
			
			int _index = Integer.parseInt(key.substring(key.length()-1));
			
			String num = String.valueOf(lastData.get(realKey).charAt(_index - 1));
			
			if(args.length > 0){
				String required = args[0];
				String notRequired = "";
				...
				if(entry.getValue().contains(required)){
					...
					if (entry.getValue().contains(notRequired)) continue;
					
					//System.out.println("Bridge [Cung vi tri] xuat hien voi tan suat >=2: "+ entry.getKey());
					System.out.println(String.format("Bridge [Cung vi tri] xuat hien voi tan suat >=2: %s ==> %s", entry.getKey(), num));
					System.out.println("Vao cac ngay: "+ entry.getValue());
				}
			}else {
				System.out.println(String.format("Bridge [Cung vi tri] xuat hien voi tan suat >=2: %s ==> %s", entry.getKey(), num));
			...
			for(Entry<String, Object> ngay_giai : tmp.entrySet()) {
				...
				for(Entry<String, String> g : giai.entrySet()) {
					...
					if(_giai.indexOf(inputDB.charAt(0)) >= 0){
						...
						System.out.println("Founds !!!!  DAU'"+brd +"<"+_giai+">");
	...
	/**
	 * Read data cua 1 ngay va dua vao Map (sDate -> Map (Giai -> Number_Of_Giai))
